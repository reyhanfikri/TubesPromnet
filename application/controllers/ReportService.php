...
class ReportService extends CI_Controller {
	...
	public function index() {
		...
		if ($this->input->post('filter') !== null) {
			...
			$config['total_rows'] = $this->MReportService->getAllReportServiceByDate($from, $to)->num_rows();
			$config['per_page'] = 50;
			...
			$data['reportservice'] = $this->MReportService->getLimitReportServiceByDate($config['per_page'], $page, $from, $to)->result();
			...
			$config['base_url'] = site_url().'ReportService/index';
			$config['total_rows'] = $this->MReportService->getAllReportServiceByMonth($month)->num_rows();
			$config['per_page'] = 50;
			$config['uri_segment'] = 3;
			$choice = $config['total_rows'] / $config['per_page'];
			$config['num_links'] = floor($choice);
			...
			$data['reportservice'] = $this->MReportService->getLimitReportServiceByMonth($config['per_page'], $page, $month)->result();
			$data['links'] = $this->pagination->create_links();
